    ...
    def f(*args, **kwargs):
        if ( not args[1] in args[0].data.keys()):
            ...
            if (size+24 > 64):
                ...
                args[0].data[args[1]] = ( data1[24:64] + data2[0:size-64+24] ).decode().encode('ascii')
            else:
                data1 = disktools.read_block(b_id)
...
def writeDataOnDisk(func):
    def f(*args, **kwargs):
        ...
        if (len(w_data) > 64):
            ...
            disktools.write_block(i, w_data[64: len(w_data)])
        else:
...
def eraseFileDisk(func):
    def f(*args, **kwargs):
        if 'block_id' in args[0].files[args[1]].keys():
            b_id =args[0].files[args[1]]['block_id']
            data = disktools.read_block(b_id)
            while(data[0] != 0):
                next_block = disktools.bytes_to_int[data[0]]
                disktools.write_block(b_id, bytearray([0]*64))
                args[0].bitmap[b_id - 4] = 0
                data = disktools.read_block(next_blocK)
                b_id = next_block
        ...
        rootMetaData = disktools.read_block(0)
        cSize = args[0].files['/']['st_size'] 
        if ( cSize > 64):
            rootMetaData += disktools.read_block(1)
        ...
        while(rootMetaData[index] != 0 ):
            ...
            if name == args[0]:
                rootMetaData = rootMetaData[0:index] + rootMetaData[index+17:cSize]
            
        if (len(rootMetaData)>64):
            disktools.write_block(0, rootMetaData[0:64])
            disktools.write_block(1, rootMetaData[64:len(rootMetaData)])
        else:
            disktools.write_block(0, rootMetaData[0:len(rootMetaData)])
